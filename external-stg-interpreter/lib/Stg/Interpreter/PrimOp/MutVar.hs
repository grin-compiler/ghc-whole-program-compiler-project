...
primOps = getPrimOpList $ do
  ...
  defOp "atomicModifyMutVar_#" $ \[MutVar m, fun, _s] -> do
    Rts{..} <- gets ssRtsSupport
    -- NOTE: CPU atomic
    old <- lookupMutVar m
  ...
  defOp "casMutVar#" $ \[MutVar m, old, new, _s] -> do
  -- NOTE: CPU atomic
