...
primOps = getPrimOpList $ do
  ...
  defOp "mkWeak#" $ \[key, value, finalizer, _w] -> do
  ...
  defOp "mkWeakNoFinalizer#" $ \[key, value, _w] -> do
    wpId <- newWeakPointer key value Nothing
    pure [WeakPointer wpId]

      -- addCFinalizerToWeak# :: Addr# -> Addr# -> Int# -> Addr# -> Weak# b -> State# RealWorld -> (# State# RealWorld, Int# #)
  defOp "addCFinalizerToWeak#" $ \[fun, dataPtr, IntV hasEnv, envPtr, WeakPointer wpId, _w] -> do
  ...
  defOp "deRefWeak#" $ \[WeakPointer wpId, _w] -> do
